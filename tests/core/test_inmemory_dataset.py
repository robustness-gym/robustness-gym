...
class TestInMemoryDataset(TestCase):
    ...
    def test_init(self):
        ...
        with self.assertRaises(AssertionError):
            InMemoryDataset.from_batch({"a": [1, 2, 3], "b": [1, 2, 3, 4]})
        ...
        dataset = InMemoryDataset.from_batch(
            {"a": [{"e": 1}, {"e": 2}, {"e": 3}, {"e": 4}], "b": ["u", "v", "w", "x"]}
        )
        self.assertEqual(len(dataset), 4)
        self.assertEqual(set(dataset.column_names), {"a", "b"})

    def test_getindex(self):
        self.assertEqual(self.dataset[0], {"a": {"e": 1}, "b": "u"})

        self.assertEqual(
            self.dataset[1:3], {"a": [{"e": 2}, {"e": 3}], "b": ["v", "w"]}
        )

        self.assertEqual(
            self.dataset[:-1],
            {"a": [{"e": 1}, {"e": 2}, {"e": 3}], "b": ["u", "v", "w"]},
        )

        self.assertEqual(
            self.dataset[::-1],
            {"a": [{"e": 4}, {"e": 3}, {"e": 2}, {"e": 1}], "b": ["x", "w", "v", "u"]},
        )

    def test_append_1(self):
        batch = {"a": [1, 2, 3, 4], "b": ["u", "v", "w", "x"]}
    ...
    def test_filter_1(self):
        ...
        self.assertEqual(dataset[:], {"a": [{"e": 1}], "b": ["u"]})
    ...
    def test_filter_2(self):
        dataset = self.dataset.filter(
            lambda example, index: example["a"] == {"e": 1}, with_indices=True
        )
    ...
    def test_filter_3(self):
        ...
        self.assertEqual(dataset_1[:], {"a": [{"e": 3}, {"e": 4}], "b": ["w", "x"]})

        self.assertEqual(dataset_2[:], {"a": [{"e": 4}], "b": ["x"]})
    ...
    def test_filter_add_column(self):
        ...
        # Dataset has the right data
        self.assertEqual(dataset[:], {"a": [{"e": 1}], "b": ["u"], "c": ["u"]})
