...
class TestConstituencyOverlap(TestCase):
    ...
    def test_has_constituency_overlap(self):
        ...
        cos = ConstituencyOverlapSubpopulation(intervals=[(0, 20), (20, 40), (40, 60), (60, 80), (80, 100)])
        self.assertTrue(np.allclose(cos.score(self.testbed.dataset[:], columns=['text_a', 'text_b']), [100] * 4))

    def test_has_constituency_subtree(self):
        # Create the constituency subtree subpopulation
        css = ConstituencySubtreeSubpopulation()
        self.assertTrue(np.allclose(css.score(self.testbed.dataset[:], columns=['text_a', 'text_b']), [1] * 4))

    def test_has_fuzzy_constituency_subtree(self):
        # Create the fuzzy constituency subtree subpopulation
        fcss = FuzzyConstituencySubtreeSubpopulation(intervals=[(0, 20), (20, 40), (40, 60), (60, 80), (80, 100)])
        self.assertTrue(np.allclose(fcss.score(self.testbed.dataset[:], columns=['text_a', 'text_b']), [100] * 4))
