...
from robustnessgym.core.tools import recmerge, persistent_hash, strings_as_json, class_or_instancemethod
...
class CachedOperation(Operation):
    ...
    def retrieve(self_or_cls,
        ...
        if not reapply:
            ...
            if 'cache' not in batch:
                raise ValueError('`cache` key missing: nothing has been cached yet. '
            ...
            if not identifier:
                ...
                # Still no identifier
                if not identifier:
                    raise ValueError(f'Retrieval failed: couldn\'t find a key called {target_ident_key} in cache.')

            try:
                if isinstance(columns[0], str):
                    retrieval = {
                        strings_as_json(columns): [
                            self_or_cls.decode(cache[str(identifier)][strings_as_json(columns)]) for cache in
                            batch['cache']
                        ]
                    }
                else:
                    retrieval = {
                        strings_as_json(cols_): [
                            self_or_cls.decode(cache[str(identifier)][strings_as_json(cols_)]) for cache in
                            batch['cache']
                        ]
                ...
                raise KeyError('Could not retrieve information for all columns. '
            ...
            if isinstance(proc_fns, str):
                ...
                proc_fns = [proc_fn if isinstance(proc_fn, Callable) else getattr(self_or_cls, proc_fn) for proc_fn in
                            proc_fns]
            ...
            if isinstance(columns[0], str):
                return {
                    strings_as_json(columns): self_or_cls(**kwargs).apply(batch=batch, columns=columns)
                    if isinstance(self_or_cls, type) else self_or_cls.apply(batch=batch, columns=columns)
                }
            return {
                strings_as_json(cols_): self_or_cls(**kwargs).apply(batch=batch, columns=cols_)
                if isinstance(self_or_cls, type) else self_or_cls.apply(batch=batch, columns=cols_)
                for cols_ in columns
            }
