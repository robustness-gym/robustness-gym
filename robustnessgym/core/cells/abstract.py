...
class AbstractCell(abc.ABC):
    ...
    def encode(self):
        """
        Encode `self` in order to specify what information is important to store.
        By default, we just return `self` so the entire object is stored.
        For complex objects (e.g. Spacy Doc), we may want to return a compressed
        representation of the object here.
        """
        return self
    ...
    def read_metadata(cls, path: str, *args, **kwargs) -> dict:
        ...
        return dict(yaml.load(open(path, 'r'), Loader=yaml.FullLoader))