...
class Schema:
    ...
    def ground(self,
        ...
        flat_columns = {tuple(path) if len(path) > 1 else path[0] for path in flat_columns}
        ...
        for k in self.reversed_grounding_candidates:
            if ((isinstance(k, tuple) or isinstance(k, str)) and k in flat_columns) \
                    or (isinstance(k, Callable)):
